...
class ListeningAudio(models.Model):
    test = models.OneToOneField(ListeningTest, on_delete=models.CASCADE, related_name='audio')
    audio_file = models.FileField(upload_to="listening_audio/")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Audio for {self.test.title}"
...
class ListeningQuestion(models.Model):
    ...
    test = models.ForeignKey(ListeningTest, related_name='questions', on_delete=models.CASCADE)
    question_type = models.CharField(max_length=50, choices=QUESTION_TYPES)
    question_text = models.TextField()
    order = models.PositiveIntegerField()
...
class ListeningAnswerKey(models.Model):
    question = models.OneToOneField(ListeningQuestion, on_delete=models.CASCADE)
    correct_answer = models.CharField(max_length=255)
...
class ListeningTestSession(models.Model):
    ...
    def __str__(self):
        return f"Listening Test Session #{self.id} - {self.user.student_id}"
    ...
    def calculate_score(self):
        ...
        """
        correct_answers_count = 0
        questions = self.test.questions.all().select_related('listeninganswerkey')
        
        # Создаем словарь правильных ответов для быстрой проверки
        correct_answers_map = {
            str(q.id): q.listeninganswerkey.correct_answer.strip().lower()
            for q in questions if hasattr(q, 'listeninganswerkey')
        }
